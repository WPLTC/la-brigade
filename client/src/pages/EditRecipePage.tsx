...
export function EditRecipePage() {
  ...
  useEffect(() => {
    if (!id) return
    apiFetch<ExistingRecipe>(`/recipes/${id}`)
      .then((recipe) => {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      ...
      if (image) {
        ...
        await apiUpload(`/recipes/${id}/image`, formData)
      }

      navigate('/profile')
    } catch (err) {
      setError(err instanceof ApiError ? err.message : 'Impossible de mettre à jour la recette')
    } finally {
    ...
    return (
      <div className="min-h-screen bg-cream">
        ...
        <main className="mx-auto max-w-2xl px-4 py-16 text-center">
          <p className="text-charcoal-light">Tu ne peux modifier que tes propres recettes.</p>
          <Link to="/profile" className="mt-4 inline-block text-brigade-red underline">
            Retour à mon profil
          </Link>
        </main>
      </div>
    ...
    <div className="min-h-screen bg-cream">
      ...
      <main className="mx-auto max-w-2xl px-4 py-10">
        ...
        <p className="mt-1 text-sm text-charcoal-light">
          Après modification, ta recette repassera en attente de validation par l'équipe du chef.
        </p>
